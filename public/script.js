...
document.addEventListener('DOMContentLoaded', () => {
    if (!isHost) {
        ...
        // Viewers go straight to the wheel page
        setTimeout(() => document.getElementById('btn-to-wheel').click(), 100);
    }
});
...
if (socket) {
    socket.on('spin', (data) => {
        if (!isHost) {
            // Ensure viewer is on the right wheel
            if (data.mode === 'church' && wheelMode !== 'church') {
                startChurchWheel();
            } else if (data.mode === 'member' && selectedChurch !== data.selectedChurch) {
...
btnSpin.addEventListener('click', () => {
    if (isSpinning || wheelData.length === 0 || !isHost) return;
    
    const totalSpins = (6 + Math.random() * 6) * Math.PI * 2;
    const duration = 5000 + Math.random() * 2000;
    const targetAngle = currentAngle + totalSpins;
    ...
    executeSpin(targetAngle, duration);
});

function executeSpin(targetAngle, duration) {
    isSpinning = true;
    if (isHost) btnSpin.disabled = true;
    ...
    function tick(now) {
        ...
        if (progress < 1) {
            ...
            isSpinning = false;
            if (isHost) btnSpin.disabled = false;
            resolveWinner();
        }
    }
